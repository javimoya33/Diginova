...
@foreach($arrArticulos as $arrArticulo)
	@if ($arrArticulo->ASTOCK > 0)
		...
		@endif
			...
			@if(session("usuario")->uData->codigo > 0)
					@if ($arrArticulo->esOferta == 1)
					<div style="width: 100%; display: flex; justify-content: space-evenly; margin-top: -35px">
						@if ($ccodcl > 0)
						...
						@endif
							...
							@if(session("usuario")->uData->codigo > 0)
								<div class="celdaTipo celda_precio_antiguo">
									{{Utils::numFormat($arrArticulo->precioAntesDeOferta)}}€
								</div>
							@endif
						</div>
					</div>
			...
			@else
				...
				@if (session("usuario")->margenesActivo)
					<div class="fPrecios ivaPresu" style="line-height: 20px; text-align: center;">
