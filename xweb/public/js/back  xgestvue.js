		...
		*/
		...
		contacto: function () {
			...
			if (this.nombreCliente.length < 6 || this.eMail.length < 3 || this.consulta.length < 3) {
				...
				alertify.alert('', "Rellene los campos del nombre de cliente, email y consulta.");
				return false;
			}
			this.rotar = true;
			$
		...
		registro: function () {
			...
			if (this.nombreCliente.length < 15) {
				...
				alertify.alert('', "Nombre de cliente demasiado corto (Mínimo 15 caracteres)");
				return false;
			}
			if (this.dni.length < 5) {
				//alert("Complete el campo NIF/CIF");
				alertify.alert('', "Inserte su NIF/CIF correctamente");
				return false;
			}
			if (this.clave1.length < 6 || this.clave2.length < 6 || this.clave2.length > 15 || this.clave1 != this.clave2) {
				...
				alertify.alert('', "Complete los campos de Nueva Contraseña para actualizar su contraseña");
				return false;
			}
			if (this.direccion.length < 15) {
				alertify.alert('', "Dirección demasiado corta (Mínimo 15 caracteres)");
				return false;
			}
			if (this.poblacion.length < 3) {
				alertify.alert('', "Inserte su Población correctamente");
				return false;
			}
			if (this.codigoPostal.length < 4) {
				alertify.alert('', "Inserte su Código Postal correctamente");
				return false;
			}
			if (this.telefono.length < 6) {
				alertify.alert('', "Inserte su Teléfono correctamente");
				return false;
			}
			if (grecaptcha.getResponse().length === 0)
			{
				alertify.alert('', "Pulse sobre 'No soy un robot' para completar el registro.");
				return false;
			}
			...
			$
				.ajax({
					...
					data: {
						...
						actividadCliente: this.actividadCliente,
					},
					type: 'POST',
					async: false,
					success: function (result) {
		...
		registro_actualizar: function () {
			...
			if (this.nombreUsuario.length < 3) {
				//alert("Complete el campo Nombre de usuario");
				alertify.alert('', "Inserte su Nombre de usuario");
				return false;
			}
			if (this.clave0.length < 1) {
				//alert("Complete el campo Clave de acceso actual");
				alertify.alert('', "Inserte su Contraseña Actual para actualizar los datos de usuario");
				return false;
			}
			if (this.clave1 != this.clave2) {
				//alert("No coinciden los datos de nueva clave");
				alertify.alert('', "No coinciden los datos de nueva Contraseña");
				return false;
			}
			this.rotar = true;
			var self = this;
