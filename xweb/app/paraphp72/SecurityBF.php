...
class SecurityBF {
	...
	function decrypt($text) {
		...
		if ($this->cbc==1) $i=2; // Message start at second block
		else
	...
	function key_setup($key) {
		...
		if (!isset($key)||strlen($key)==0) $key=array(
				0);
		else if (!is_array($key)) $key=$this->_str2long(str_pad($key,strlen($key)+strlen($key)%4,$key));
	...
	function check_implementation() {
		...
		foreach ( $vectors as $vector ) {
			...
			if ($return[0]!=$cipher[0]||$return[1]!=$cipher[1]) $correct=false;
		}
	...
	function block_encrypt($v0, $v1) {
		...
		for($i=0; $i<16; $i++) {
			...
			$v0=((($this->sbox0[($temp>>24)&0xFF]+$this->sbox1[($temp>>16)&0xFF])^$this->sbox2[($temp>>8)&0xFF])+$this->sbox3[$temp&0xFF])^$v1;
